...
//                        (atms.getRAIN() + soil.getSNOWINF() - soil.getEET() ),
    ...
    soil.setNLOST( soil.getNLOST() + 0.01*microbe.getGMIN() );
                             ...
                             const double& psiplusc )
{
  ...
  y[I_DOC] =  solcb[pdcmnt];

  if( y[I_DOC] < ZERO ) { y[I_DOC] = ZERO; }

  y[I_DON] =  solnb[pdcmnt];

  if( y[I_DON] < ZERO ) { y[I_DON] = ZERO; }
 ...
 if( disturbflag ==  1 && pdm == (disturbmonth-1)) // agriculture
  {
    ...
    prevy[I_SAPWOODC] = y[I_SAPWOODC] = (1-(ag.getPROD10PAR() + ag.getPROD100PAR() + ag.getVCONVERT())) * y[I_SAPWOODC];
    prevy[I_HEARTWOODC] = y[I_HEARTWOODC] = (1-(ag.getPROD10PAR() + ag.getPROD100PAR() + ag.getVCONVERT()))*y[I_HEARTWOODC];
    prevy[I_LABILEC] = y[I_LABILEC] = (1-(ag.getPROD10PAR() + ag.getPROD100PAR() + ag.getVCONVERT()))*y[I_LABILEC];
    prevy[I_SAPWOODN] = y[I_SAPWOODN] = (1-(ag.getPROD10PAR() + ag.getPROD100PAR() + (1-ag.getNVRETCONV(veg.cmnt))*ag.getVCONVERT()))*y[I_SAPWOODN];
    prevy[I_HEARTWOODN] = y[I_HEARTWOODN] = (1-(ag.getPROD10PAR() + ag.getPROD100PAR() + (1-ag.getNVRETCONV(veg.cmnt))*ag.getVCONVERT()))*y[I_HEARTWOODN];
    prevy[I_LABILEN] = y[I_LABILEN] = (1-(ag.getPROD10PAR() + ag.getPROD100PAR() + (1-ag.getNVRETCONV(veg.cmnt))*ag.getVCONVERT()))*y[I_LABILEN];
  }
...
else if ( disturbflag ==  2 && pdm == (disturbmonth-1)) // timber harvest clearcut
  {
    ...
    prevy[I_LABILEC] = y[I_LABILEC] = (1. - timber)*(1-(ag.getPROD10PAR() + ag.getPROD100PAR()))* y[I_LABILEC];
    prevy[I_LABILEN] = y[I_LABILEN] = (1. -  timber)*(1-(ag.getPROD10PAR() + ag.getPROD100PAR()))*y[I_LABILEN];
    prevy[I_SAPWOODC] = y[I_SAPWOODC] = (1. - timber)*(1-(ag.getPROD10PAR() + ag.getPROD100PAR()))*y[I_SAPWOODC];
    prevy[I_SAPWOODN] = y[I_SAPWOODN] = (1. - timber)*(1-(ag.getPROD10PAR() + ag.getPROD100PAR()))*y[I_SAPWOODN];
    prevy[I_HEARTWOODC] = y[I_HEARTWOODC] = (1. - timber)*(1-(ag.getPROD10PAR() + ag.getPROD100PAR()))*y[I_HEARTWOODC];
    prevy[I_HEARTWOODN] = y[I_HEARTWOODN] = (1. - timber)*(1-(ag.getPROD10PAR() + ag.getPROD100PAR()))*y[I_HEARTWOODN];
   }
...
////
      ...
      prevy[I_LABILEC] = y[I_LABILEC] = (1. - dwood)*(1- ag.getVCONVERT())*y[I_LABILEC];
      prevy[I_LABILEN] = y[I_LABILEN] = (1. - dwood) * y[I_LABILEN];
      prevy[I_LEAFC] = y[I_LEAFC] = (1 - ag.getVCONVERT())* (1. - dleaf)*y[I_LEAFC];
      prevy[I_LEAFN] = y[I_LEAFN] = (1 - ag.getVCONVERT())* (1. - dleaf)*y[I_LEAFN];
      prevy[I_SAPWOODC] = y[I_SAPWOODC] = (1. - dwood)*(1- ag.getVCONVERT())*y[I_SAPWOODC];
      prevy[I_SAPWOODN] = y[I_SAPWOODN] = (1. - dwood)*(1- ag.getVCONVERT())*y[I_SAPWOODN];
      prevy[I_HEARTWOODC] = y[I_HEARTWOODC] = (1. - dwood)*(1- ag.getVCONVERT())*y[I_HEARTWOODC];
      prevy[I_HEARTWOODN] = y[I_HEARTWOODN] = (1. - dwood)*(1- ag.getVCONVERT())*y[I_HEARTWOODN];
      prevy[I_ROOTC] = y[I_ROOTC] = (1 - ag.getVCONVERT()) * (1. - dwood)*y[I_ROOTC];
      prevy[I_ROOTN] = y[I_ROOTN] = (1 - ag.getVCONVERT()) * (1. - dwood)*y[I_ROOTN ];
  }
