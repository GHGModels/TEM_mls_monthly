...
////
  ...
  if(initFlag == 1 && pdm == 7 && (veg.cmnt == 5 || veg.cmnt == 6 || veg.cmnt == 4 || veg.cmnt == 10 || veg.cmnt == 11 || veg.cmnt == 12))
  {
  ...
  ag.setNATSEEDSTON (y[I_SEEDN] );
      ...
      ag.standingdead( veg.cmnt,
                 0.205*y[I_LEAFC]+0.04*(y[I_ROOTC]+y[I_LABILEC]),
                 0.205*y[I_LEAFN]+0.04*(y[I_ROOTN]+y[I_LABILEN]),
                 (0.04*(y[I_SAPWOODC]+y[I_HEARTWOODC])),
                 (0.04*(y[I_SAPWOODN]+y[I_HEARTWOODN])) );
  ...
  prevy[I_LEAFC] = y[I_LEAFC] = (1. - 0.205)*y[I_LEAFC];
  prevy[I_LEAFN] = y[I_LEAFN] = (1. - 0.205)*y[I_LEAFN];
  prevy[I_SAPWOODC] = y[I_SAPWOODC] = (1. - 0.04)*y[I_SAPWOODC];
  prevy[I_SAPWOODN] = y[I_SAPWOODN] = (1. - 0.04)*y[I_SAPWOODC];
  prevy[I_HEARTWOODC] = y[I_HEARTWOODC] = (1. - 0.04)*y[I_HEARTWOODC];
  prevy[I_HEARTWOODN] = y[I_HEARTWOODN] = (1. - 0.04)*y[I_HEARTWOODC];
  ...
  ag.setNATSEEDSTON (y[I_SEEDN] );
      ...
      prevy[I_LABILEC] = y[I_LABILEC] = (1. - 0.08)*y[I_LABILEC];   //destruction values from Batista & Platt
      prevy[I_LABILEN] = y[I_LABILEN] = (1. - 0.08)*y[I_LABILEN];
      prevy[I_LEAFC] = y[I_LEAFC] = (1. - 0.41)*y[I_LEAFC];
      prevy[I_LEAFN] = y[I_LEAFN] = (1. - 0.41)*y[I_LEAFN];
      prevy[I_SAPWOODC] = y[I_SAPWOODC] = (1. - 0.08)*y[I_SAPWOODC];
      prevy[I_SAPWOODN] = y[I_SAPWOODN] = (1. - 0.08)*y[I_SAPWOODC];
      prevy[I_HEARTWOODC] = y[I_HEARTWOODC] = (1. - 0.08)*y[I_HEARTWOODC];
      prevy[I_HEARTWOODN] = y[I_HEARTWOODN] = (1. - 0.08)*y[I_HEARTWOODC];
      prevy[I_ROOTC] = y[I_ROOTC] = (1. - 0.08)*y[I_ROOTC];
      prevy[I_ROOTN] = y[I_ROOTN] = (1. - 0.08)*y[I_ROOTN];
   }
  }
  ...
  if(initFlag == 1 && pdm == 7 && ag.state != 1)
  {
      double dleaf=0,dwood=0;
      int repi=0,mixi=0,lowi=0;

      if(veg.cmnt == 2) // alpine tundra
      {
        lowi = 0;
        mixi = 750;
        repi = 350;
      ...
      if(veg.cmnt == 4) // boreal forest
      {
        lowi = 0;
        mixi = 1000;
        repi = 217;
      }
      if(veg.cmnt == 5) // temperate coniferous
      {
        lowi = 13;
        mixi = 60;
        repi = 200;
      }
      if(veg.cmnt == 6) // temperate deciduous
      {
        lowi = 11;
        mixi = 77;
        repi = 66;
      }
      if(veg.cmnt == 7) // tall grassland
      {
        lowi = 28;
        mixi = 34;
        repi = 5;
      }
      if(veg.cmnt == 8) // short grassland
      {
        lowi = 0;
        mixi = 0;
        repi = 8;
      }
      if(veg.cmnt == 9) // arid shrublands
      {
        lowi = 0;
        mixi = 200;
        repi = 50;
      ...
      if(veg.cmnt == 11) // xeric forests
      {
        lowi = 14;
        mixi = 140;
        repi = 65;
      }
      if(veg.cmnt == 12) // temperate broadleaved evergreen
      {
        lowi = 45;
        mixi = 25;
        repi = 140;
      }
      if(veg.cmnt == 13) // Mediterranean shrublands
      {
        lowi = 0;
        mixi = 0;
        repi = 50;
