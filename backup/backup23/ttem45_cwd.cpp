...
// Ecosystem nitrogen pools determined by the integrator********
  ...
  predstr.at( I_AVLNFB ) = "AVAILNFB";    //MJ MLS;
...
// Nitrogen fluxes for ecosystems determined by the integrator
  ...
  predstr.at( I_DOCPRODFB ) = "DOCPRODFB";   //MJ MLS;
  ...
  predstr.at( I_DONPRODFB ) = "DONPRODFB";   //MJ MLS;
  ...
  predstr.at( I_MNUPFB ) = "MICRONUPFB";   //MJ MLS;
  ...
  predstr.at( I_NMINFB ) = "NETNMINFB";   //MJ MLS;
...
// Nitrogen fluxes associated with agricultural conversion *****
  ...
  predstr.at( I_NSRTNTFB ) = "NSRETENTFB";       //added MJ MLS;
...
nopen = 0; 
  ...
  microbe.updateDynamics( ag.cmnt,
                          ...
                          soil.getPCTPOR(),
                          soil.getSOLC(),
                          pstate[I_FBC],    //come back to swap with SOLC MJ MLS;
                          pstate[I_AMC],
                          pstate[I_MNC],
                          soil.getSOLN(),
                          pstate[I_FBN],    //come back to swap with SOLN MJ MLS;
                          pstate[I_AMN],
                          pstate[I_MNN],
                          pstate[I_SM],
                          pstate[I_VSM],
...
//                     pstate[I_AVLN],
                     ...
                     ag.fertn,
                     soil.getSONINP() );
   soil.setSONINP(veg.getSONINPUT());    //commented out for the following two lines MJ MLS, come back to check the possibility of SONINP = SONINPFB + SONINPAM;
//   soil.setSONINPFB(veg.getSONINPUTFB());
//   soil.setSONINPAM(veg.getSONINPUTAM());
...
// nin = soil.getNINPUT() + soil.getSONINP() + veg.getVEGNINP() + ag.getSTUBBLEN();
  nin = soil.getNINPUT() + soil.getSONINP() + veg.getVEGNINP() + nseed;     //commented out for MJ MLS;
//  nin = soil.getNINPUT() + soil.getSONINPFB() + soil.getSONINPAM() + veg.getVEGNINP() + nseed;   //MJ MLS;
...
//         + microbe.getNETNMIN() + soil.getNINPUT() )
    {
//      cout << "Nlost = " << soil.getNLOST() << " " << pstate[I_AVLN] << " " << veg.getNUPTAKE() << " " << microbe.getNETNMIN() << " " << soil.getNINPUT() << endl;
...
//For the AMC and MNC, missing the depth-threshold dependent incoming C fluxes from the FBC and AMC, come back to add these MJ MLS;
...
ag.setSLASHN(0.0);
  ...
  pdstate[I_AVLNAM] = soil.getNINPUT()
                    + microbe.getNETNMINAM()
                    - veg.getNUPTAKE()
                    - soil.getNLOST();

  pdstate[I_AVLNMN] = soil.getNINPUT()
                    + microbe.getNETNMINMN()
                    - veg.getNUPTAKE()
                    - soil.getNLOST();

//cout << "diag = " << soil.getNINPUT() << " " << microbe.getNETNMIN() << " " <<  veg.getNUPTAKE() << " " << soil.getNLOST() << endl;
...
//#endif  
  ...
  pdstate[I_MNUPFB] = microbe.getNUPTAKEFB();    //MJ MLS following example of NETNMIN;
  pdstate[I_MNUPAM] = microbe.getNUPTAKEAM();
  pdstate[I_MNUPMN] = microbe.getNUPTAKEMN();

  pdstate[I_NMIN] = microbe.getNETNMIN();     //come back to remove this line MJ MLS;
...
//  if( microbe.getKDB( dcmnt ) <= -99.99 ) { return qc = 146; }
  if( microbe.getKDFS( dcmnt ) <= -99.99 ) { return qc = 145; }  
  if( microbe.getKDSLOW( dcmnt ) <= -99.99 ) { return qc = 146; }
...
/*                                            //commented out because SOLN is set to be the sum of FBN, AMN and MNN MJ MLS;
...
//added MJ MLS;
  ...
  y[I_AVLNFB] = avlnfb[pdcmnt];                          //MJ MLS added;
...
double Ttem45::getOptionalEflx( const int& optflx )
{
  ...
  switch( optflx )
  {
    ...
    case GET_SOILC:   outflux = y[I_SOLC]; break;        //since SOLC left in xtem, keep SOLC in ctem for now MJ MLS
...
*/  //commented out for MJ MLS;
  microbe.setKDFS( veg.getXMLcmntArrayDouble( fecd[dv],
                                             "siteECD",
                                             "microbekdfs",
                                             veg.cmnt ),
                  veg.cmnt );

  microbe.setKDSLOW( veg.getXMLcmntArrayDouble( fecd[dv],
                                             "siteECD",
                                             "microbekdslow",
                                             veg.cmnt ),
                  veg.cmnt );
...
/*
    y[I_RHFB] = y[I_LTRLC]
              ...
              + y[I_LTRRC]*soil.getFRROOTC_FB(veg.cmnt)
              + y[I_LTRSEEDC]
              - 0.076 * y[I_FBC]            //coefficient is not correct, should be decomposition rate for the fast C pool (KDfast);
              - 0.125 * y[I_FBC]            //coefficient is not correct, should be decomposition rate for the fast C pool (KDfast);
              - 0.0035 * y[I_FBC]           //coefficient is not correct, should be decomposition rate for the fast C pool (KDfast);
              - y[I_DOCPRODFB]
              + ag.getSLASHC()
              - ag.getSCONVRTFLXC()
              - y[I_FBC]
              + prevy[I_FBC];              //missing the portion of C added into amorphous horizon when certain depth threshold is passed;
*/
...
//come back to modify with multiple horizons MJ MLS;
...
//  soil.setNINPUT((0.102 * (12.0*soil.getEET()/10.0)+ 0.524 )/(10.0*12.0));
  ...
  soil.setSONINP((veg.getNNF(veg.cmnt)*0.102 * (12.0*soil.getEET()/10.0)+ 0.524 ) /(10.0*12.0));     //removed for MJ MLS;
...
// Runge-Kutta-Bogacki-Shampine method: compares 2nd and 3rd order solutions
  ...
  if( calc_dz1_flag != 0 ) { delta( pdm, pstate, dz1 ); }
...
//    if(i == 34) {cout << "diag = " << ptstate[i] << endl;}
  }
...
//cout << "entering stepmonth " << veg.cmnt << " " << pdyr << " " << pdm << " " << y[I_GPP] << endl;
  ...
  soil.setSOLC( y[I_FBC] + y[I_AMC] + y[I_MNC] );   //commented out for now MJ MLS;
  soil.setSOLN( y[I_FBN] + y[I_AMN] + y[I_MNN] );   //MJ MLS;
  soil.setAVLN( y[I_AVLNFB] + y[I_AVLNAM] + y[I_AVLNMN] );   //MJ MLS;
...
//  }
  ...
  if( 0 == pdm )
  {
    if( 0 == pdyr )
    {
      ...
      ag.setNATSOIL( soil.getSOLC() );        //should we use a single soilC or multiple horizon soilC here? MJ MLS
    }
    else
    {
...
//    if( 1 == baseline || microbe.getDOCFR( veg.cmnt ) == 0 )
    if( 1 == baseline )
    {
      ...
      y[I_FBN] = y[I_FBN] + soil.yrnin - soil.yrnlost;
//    y[I_SOLN] = (y[I_FBN] + y[I_AMN] + y[I_MNN]) + soil.yrnin - soil.yrnlost;    //come back to uncomment this line MJ MLS;
     }
...
//  } 
  ...
  soil.yravln = soil.yravln*(1.0 - favg) + soil.getAVLN()*favg;
