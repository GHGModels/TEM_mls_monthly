...
//    soilceq[(xdyr%de_nyears)] = tem.getY(tem.I_SOLC);    //OK or not? MJ MLS;
    soilceq[(xdyr%de_nyears)] = tem.soil.getSOLC();     //replacing the above line MJ MLS,
    vegneq[(xdyr%de_nyears)] = tem.veg.getVEGN();
//    soilneq[(xdyr%de_nyears)] = tem.getY(tem.I_SOLN);
    soilneq[(xdyr%de_nyears)] = tem.soil.getSOLN();      //replacing the above line MJ MLS;
  ...
  if( xdyr%((int)(5*tem.tauavg)) == 0 )
  {
    ...
    switch( tempcalstep )
    {
      ...
      case 9: // check that npp ~ npp_t and gpp ~gpp_t
        ...
        break;          
//if kdc is commented out in tmcrb, then comment out case 10, otherwise, keep case 10 MJ MLS;
